...
class CommonsenseRNNCell(nn.Module):
    ...
    def forward(self, U, x1, x2, x3, o1, o2, qmask, g_hist, q0, r0, i0, e0):
        ...
        ## external state ##
        U_r_c_ = torch.cat([U, x2, c_], dim=1).unsqueeze(
        ...
        ## internal state ##
        es_c_ = torch.cat([x1, c_], dim=1).unsqueeze(
            1).expand(-1, qmask.size()[1], -1)
        ...
        if self.listener_state:
            ...
            er_ = o2.unsqueeze(1).expand(-1, qmask.size()
                                         [1], -1).contiguous().view(-1, self.D_s)
            ...
            rl_ = self.rl_cell(U_er_ss_, r0.view(-1, self.D_r)
                               ).view(U.size()[0], -1, self.D_r)
            # rl_ = self.dropout(rl_)

            ## listener internal state ##
            es_ = o1.unsqueeze(1).expand(-1, qmask.size()
                                         [1], -1).contiguous().view(-1, self.D_s)
            ss_ = self._select_parties(qs_, qm_idx).unsqueeze(1).\
                expand(-1, qmask.size()[1], -1).contiguous().view(-1, self.D_p)
            es_ss_ = torch.cat([es_, ss_], 1)
            ql_ = self.pl_cell(es_ss_, q0.view(-1, self.D_p)
        ...
        ## intent ##
        i_q_ = torch.cat([x3, self._select_parties(q_, qm_idx)], dim=1).unsqueeze(
            1).expand(-1, qmask.size()[1], -1)
...
class CommonsenseRNN(nn.Module):
    ...
    def forward(self, U, x1, x2, x3, o1, o2, qmask):
        ...
        q_ = torch.zeros(qmask.size()[1], qmask.size()[2], self.D_p).type(
            U.type())  # batch, party, D_p
        r_ = torch.zeros(qmask.size()[1], qmask.size()[2], self.D_r).type(
            U.type())  # batch, party, D_r
        i_ = torch.zeros(qmask.size()[1], qmask.size()[2], self.D_i).type(
            U.type())  # batch, party, D_i
...
class CommonsenseGRUModel(nn.Module):
    ...
    def __init__(self, D_m, D_s, D_g, D_p, D_r, D_i, D_e, D_h, D_a=100, n_classes=7, listener_state=False,
        ...
        self.cs_rnn_r = CommonsenseRNN(D_h, D_s, D_g, D_p, D_r, D_i, D_e, listener_state,
                                       context_attention, D_a, dropout_rec, emo_gru)
        self.sense_gru = nn.GRU(
            input_size=D_s, hidden_size=D_s//2, num_layers=1, bidirectional=True)
    ...
    def forward(self, r1, r2, r3, r4, x1, x2, x3, o1, o2, qmask, umask, att2=False, return_hidden=False):
        ...
        if self.norm_strategy == 1:
            r1 = self.norm1a(r1.transpose(0, 1).reshape(-1, feature_dim)
                             ).reshape(-1, seq_len, feature_dim).transpose(1, 0)
            r2 = self.norm1b(r2.transpose(0, 1).reshape(-1, feature_dim)
                             ).reshape(-1, seq_len, feature_dim).transpose(1, 0)
            r3 = self.norm1c(r3.transpose(0, 1).reshape(-1, feature_dim)
                             ).reshape(-1, seq_len, feature_dim).transpose(1, 0)
            r4 = self.norm1d(r4.transpose(0, 1).reshape(-1, feature_dim)
                             ).reshape(-1, seq_len, feature_dim).transpose(1, 0)
            ...
            r1 = self.norm3a(r1.transpose(0, 1).reshape(-1, feature_dim)
                             ).reshape(-1, seq_len, feature_dim).transpose(1, 0)
            r2 = self.norm3b(r2.transpose(0, 1).reshape(-1, feature_dim)
                             ).reshape(-1, seq_len, feature_dim).transpose(1, 0)
            r3 = self.norm3c(r3.transpose(0, 1).reshape(-1, feature_dim)
                             ).reshape(-1, seq_len, feature_dim).transpose(1, 0)
            r4 = self.norm3d(r4.transpose(0, 1).reshape(-1, feature_dim)
                             ).reshape(-1, seq_len, feature_dim).transpose(1, 0)
        ...
        if self.mode1 == 0:
            ...
            r = self.r_weights[0]*r1 + self.r_weights[1] * \
                r2 + self.r_weights[2]*r3 + self.r_weights[3]*r4
        ...
        emotions_b, alpha_b = self.cs_rnn_r(
            rev_r, rev_x1, rev_x2, rev_x3, rev_o1, rev_o2, rev_qmask)
